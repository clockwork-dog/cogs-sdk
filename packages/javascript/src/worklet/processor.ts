...
import { PHASE_VOCODER_PROCESSOR_NAME } from './processorName';
...
export interface AudioWorkletProcessorOptions {
  ...
  outputChannelCount?: [number, number];
  parameterData?: { parameters?: Record<string, any> };
...
export interface OLAProcessorOptions extends AudioWorkletProcessorOptions {
  numberOfInputs: number;
  numberOfOutputs: number;
  processorOptions: {
    blockSize: number;
...
class OLAProcessor extends AudioWorkletProcessor {
  ...
  constructor(options: OLAProcessorOptions) {
    super(options);
  ...
  reallocateChannelsIfNeeded(inputs: Float32Array[][], outputs: Float32Array[][]) {
    for (let i = 0; i < this.nbInputs; i++) {
  ...
  process(inputs: Float32Array[][], outputs: Float32Array[][], params: OLAProcessorOptions['parameterData']) {
  ...
  processOLA(_inputs: Float32Array[][], _outputs: Float32Array[][], params: any) {
...
interface PhaseVocoderProcessorOptions extends OLAProcessorOptions {
  parameterData: {
    parameters: {
      pitchFactor: number[];
    };
...
class PhaseVocoderProcessor extends OLAProcessor {
  ...
  fft: FFT;
  freqComplexBuffer: number[];
  freqComplexBufferShifted: number[];
  timeComplexBuffer: number[];
  magnitudes: Float32Array;
  peakIndexes: Int32Array;
  nbPeaks: number;

  constructor(options: PhaseVocoderProcessorOptions) {
  ...
  processOLA(inputs: Float32Array[][], outputs: Float32Array[][], parameters: PhaseVocoderProcessorOptions['parameterData']['parameters']) {
...
registerProcessor(PHASE_VOCODER_PROCESSOR_NAME, PhaseVocoderProcessor);
